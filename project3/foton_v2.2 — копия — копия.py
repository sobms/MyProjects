...
class App(QtWidgets.QMainWindow, foton_display4.Ui_MainWindow):
    ...
    def renew_table(self, table2):
        Max_Row = table2.shape[0]
        for row in range(0,Max_Row):
            for column in range(0,6):
                item = QtWidgets.QTableWidgetItem()
                self.tableWidget.setItem(row, column, item)
                item.setText(str(table2.iloc[row,column]))
                de_plus = []
        de_minus = []
        for i in range(0,Max_Row):
                self.E_ey[i] = self.table['Энергия_излуч.'].iloc[i]*self.S/1000000
                self.P[i] = self.table['Нагрузка'].iloc[i]
                x = (self.E_ey[i]*self.N_min-self.P[i]/self.Nu_i)
                if x>0:
                    E_ey = x*(self.a1*(self.E_ey_nom-x/self.k)+self.b1)
                    E_cv = self.P[i]/self.Nu_i
                    E_te = 0
                    #de_plus.append(np.round_((dE*self.Nu_i*(self.a1*(self.E_ey_nom-self.E_ey[i]*self.N_min)+self.b1)),4))
                    #de_minus.append(0)
                elif x<=0:
                    E_ey = 0
                    E_cv = self.table['Энергия_излуч.'].iloc[i]*self.S/1000000*self.N_min*self.Nu_i
                    E_te = (self.P[i]-E_cv)/(self.a2*(self.E_te_nom-E_cv/self.k2)+self.b2)
    ...
    def CalcD_n(self, table, n):
        ...
        for i in range(0,Max_Row):
            ...
            if x>0:
                    ...
                    E_cv = self.table['Энергия_излуч.'].iloc[i]*self.S/1000000*self.N_min*self.Nu_i
                    E_te = (self.P[i]-E_cv)/(self.a2*(self.E_te_nom-E_cv/self.k2)+self.b2)
            sum_ += (E_ey-E_te-self.E_vp/(self.a1*(self.E_ey_nom-x/self.k)+self.b1))
        D_n = np.abs(sum_)
        return D_n  
    ...
    def get_year_dE(self):
        ...
        for i in range(0,12):
            ...
            for i in range(0,data.shape[0]):
                ...
                if dE<=0:
                    F2+= (-dE)/(self.a2*(P_max-dE) + self.b2)
            y.append(F1-F2-self.table.shape[0]*self.E_vp)
        fig = Figure(figsize=(5,1.5))
        fig.patch.set_facecolor('#b6c3be')
        ax = fig.add_subplot()
    ...
    def Count_N_min(self):
        ...
        self.label_31.setText(str(self.P_max))
        ...
        for i in range(0,12):
            ...
            for i in range(0,data.shape[0]):
                ...
                if dE>0:
                    F1+= dE*(self.a1*(dE-self.E_ey_nom) + self.b1)
                if dE<0:
                    F2+= -dE/(self.a2*(self.P_max-dE) + self.b2)
            y.append(F1-F2-self.table.shape[0]*self.E_vp)
    ...
    def Calc_ey_num(self):
        try:
            ...
            self.label_34.setText(str(np.round_(v_h2/eff)))
        except:
            print("Error")
    ...
    def Calc_te_num(self):
        try:
            ...
            self.label_36.setText(str(np.round_((np.abs(np.min(y)))/n_te_nom)))
        except:
            print("Error")    
    ...
    def Calc_stoves(self):
        ...
        table2['Нагрузка'] = table2['Нагрузка'] - self.E_vp
        self.renew_table(table2)
        self.Count_N_min()
        self.Calc_ey_efficiency()
        self.Calc_ey_num()
        self.Calc_te_num()
        self.Calc_num_bal_and_max_mass()
        self.label_48.setText(str(1.5*t_stoves*n_hotplate/24*8784*0.42))
    ...
    def Calc_boiler(self):
        ...
        self.Calc_te_num()
        self.Calc_num_bal_and_max_mass()
        self.label_48.setText(str(S*0.1*24*30*7/2*1.67))

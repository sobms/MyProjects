...
class App(QtWidgets.QMainWindow, foton_display4.Ui_MainWindow):
    ...
    def get_daily_power(self, month, day):
        ...
        ax = fig.add_subplot()
        ax.set_title('Самый благоприятный день по приходу солнечной энергии ' + str(day) + ' число ' + str(month) + ' мес', fontsize = 12, horizontalalignment='center')
        ax.set_xlabel('Время суток, час', fontsize=10)
        ax.set_ylabel('Мощность, кВт', fontsize=10)
        ax.grid()
        ax.patch.set_facecolor('#e6e6e6')
        ax.tick_params(axis = 'both', which = 'major', labelsize = 9)
        ax.xaxis.set_major_locator(ticker.MultipleLocator(1))
        ax.annotate(str(np.round_(sum_))+" кВт*ч", xy=(0, (np.max(y1)-np.min(y1))/10*9), fontsize = 9)
    ...
    def get_daily_workload(self, month, day):
        ...
        ax = fig.add_subplot()
        fig.patch.set_facecolor('#b6c3be')
        ax.set_title('Суточный график нагрузки ' + str(day) + ' число ' + str(month) + ' мес', fontsize = 12, horizontalalignment='center')
        ax.set_xlabel('Время суток, ч', fontsize=10)
        ax.set_ylabel('Нагрузка, кВт', fontsize=10)
        ax.annotate(str(np.round_(sum_))+" кВт*ч", xy=(0, (np.max(y)-np.min(y))/10*9), fontsize = 9)
        ax.grid()
        ax.patch.set_facecolor('#e6e6e6')
        ax.tick_params(axis = 'both', which = 'major', labelsize = 9)
        ax.xaxis.set_major_locator(ticker.MultipleLocator(1))
    ...
    def get_year_dE(self):
        ...
        fig.patch.set_facecolor('#b6c3be')
        ax = fig.add_subplot()
        ax.set_title('Годовой баланс энергии запасенной в водороде', fontsize = 12, horizontalalignment='center')
        ax.set_xlabel('Месяц', fontsize=10)
        ax.set_ylabel('dE, кВт*ч', fontsize=10)
        ax.grid()
        ax.tick_params(axis = 'both', which = 'major', labelsize = 9)
    ...
    def get_year_balance(self):
        ...
        ax = fig.add_subplot()
        fig.patch.set_facecolor('#b6c3be')
        ax.set_title('Годовой баланс энергии, запасенной в водороде', fontsize = 18)
        ax.set_xlabel('Месяц', fontsize=15)
        ax.set_ylabel('dE, кВт*ч', fontsize=15)
        ax.patch.set_facecolor('#e6e6e6')
    ...
    def Calc_ey_num(self):
        try:
            ...
            self.label_34.setText(str(int(np.ceil(v_h2/eff))))
        except:
            print("Error")
    ...
    def Calc_te_num(self):
        try:
            ...
            self.label_36.setText(str(int(np.ceil((np.abs(np.min(y)))/n_te_nom))))
        except:
            print("Error")    
        ...
        for i in range(0,12):
            ...
            for i in range(0,data.shape[0]):
                ...
                if dE<0:
                    F2+= (-dE-self.E_vp)/(self.a2*(self.P_max-dE) + self.b2)+self.E_vp
            y.append(F1-F2)
        return int(np.round_(np.max(y)/(p_h2*v_bal*A_v)))
    ...
    def Calc_stoves(self):
        ...
        self.label_48.setText(str(np.round_(1.5*t_stoves*n_hotplate/24*365*0.42,2)))
